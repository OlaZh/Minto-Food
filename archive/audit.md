...
> **Статус (2026-06-16): ✅ C1, C2, C3 ЗРОБЛЕНО; C4 система B готова.** z-шкала (коміт 9dc6a40), плейсхолдери (eb68cfb). C4: ✅ report-modal (b44f3dc), ✅ book-selector (294fe8e), ✅ довгі модалки системи B через глобальний extend (b20c1b7) — усі перевірені в браузері. ⏭️ свідомо пропущено як структурно-окремі: **scanner** і **система A** (`.modal`/`hidden`, інвертований показ + radial-gradient). **C3 закрито (2026-06-16):** спільні `openModal/closeModal/initModal/initTabs` виявились МЕРТВИМИ (0 імпортів) → видалені; деталі в C3 нижче.
>
> **🎨 ОНОВЛЕНО (2026-06-27): візуальну уніфікацію ЗАКРИТО — але в ПРОТИЛЕЖНИЙ бік.** Замість зводити родину A до flat (як спершу припускав аудит), за рішенням власника **систему B підтягнуто ВГОРУ до вигляду родини A**: `%overlay`/`%modal-card` ([_design-system.scss](scss/utils/_design-system.scss)) озброєно radial-gradient фоном (`circle at top`, +dark-варіант), `blur(6px)` і градієнтною карткою (gradient surface + бордер + тінь `0 32px 64px`). Уся система B extend-ить ці плейсхолдери (з C4) → оновилась автоматично, **0 JS-змін, 0 правок кожної модалки**. Доведено дифом скомпільованого css: змінились лише `.report-overlay`/`.book-selector-overlay`/`.modal-overlay` + їхні картки; **родина A (`.modal`/`.scanner-modal`/`.auth-modal`) у дифі відсутня — не зачеплена.** Тобто весь видимий «3 системи виглядають по-різному» борг закрито.
>
> **🔧 ОНОВЛЕНО (2026-06-27): JS-контракт показу родини A ТЕЖ уніфіковано → C ПОВНІСТЮ ЗАКРИТО.** Показ модалок родини A переведено з атрибута `hidden` на клас `.is-open` (як у системі B). Обсяг свідомо вузький: **лише контракт показу самих модалок**, внутрішній `[hidden]` (таби логін/реєстрація, error/success, scanned-card, fiberRow) НЕ чіпали — це легітимний семантичний HTML, не борг. Змінено:
> - **CSS (3 правила):** `.modal`/`.scanner-modal`/`.auth-modal` — дефолт `display:none` + `.is-open{display:flex}` замість `[hidden]`. Доведено дифом css: змінились лише ці 3 show-правила, вигляд/внутрішні елементи не зрушено.
> - **HTML (6 контейнерів):** прибрано атрибут `hidden` з addFoodModal/createProductModal/scannerModal (index.html), product-modal (product-guide.html), weekAddModal (week-menu.html), auth-modal (JS-шаблон у auth.js).
> - **JS (12 точок, 5 файлів):** `el.hidden=false/true` → `el.classList.add/remove('is-open')` у meals.js (4), barcode-scanner.js (2), product-guide.js (2), week-menu.js (2), auth.js (2). `node --check` усіх — OK. `showConfirmModal` ([ui-components.js:151](js/ui-components.js#L151)) свідомо НЕ чіпали — це confirm-modal (не родина A), уже на `.is-active`.
> - ⏳ **Перевірка в браузері (накопичено):** відкрити/закрити кожну з 6 модалок (food-modal найважливіша), Esc, клік по фону, перемикання табів логін↔реєстрація в auth.
...
- ✅ **close-кнопки системи B** — ЗРОБЛЕНО (2026-06-17). `.report-modal__close`, `.book-selector__close` і глобальний `.modal-card__close` зведені в спільний `%modal-close` у `scss/utils/_design-system.scss`; селектори й поведінка не змінені, лише прибрано дубль.
...
>
> **✅ Close-кнопки (з C4):** `.report-modal__close` + `.book-selector__close` + глобальний `.modal-card__close` вже зведені в `%modal-close` без зміни контракту.
...
- 🟨 **Профіль (profile.html)** — ІТЕРАЦІЯ 4, частина А (2026-06-16): **вкладка «Налаштування» + перемикач ЗРОБЛЕНІ.**
  ...
  - 🟨 **Частина Б — СТАТИКА решти вкладок ЗРОБЛЕНА (2026-06-16).** Розмічено data-i18n: sidebar-таби (Мої дані/Контроль ваги/Активність/Статистика/Налаштування), h1, вкладки «Мої дані» (параметри/стать/ціль/активність-рівні/денна норма/макроси/ІМТ), «Контроль ваги» (×2 десктоп+мобайл: цілі/вага/динаміка/поради/прогрес), «Активність» (форма/типи-лейбли/Сьогодні-статистика/баланс/історія/період-фільтр), «Статистика» (6 карток: Баланс БЖВ/Динаміка калорій/Розподіл/Порівняння/Топи/Рекомендації + бейджі/легенда/compare). Заголовки з svg → текст у `<span>`. **+65 ключів ×3 мови** (зокрема ті, що мали data-i18n у HTML, але БЕЗ ключів у словнику — раніше не працювали: yourParameters/age/gender/goal*/activity*/updateData/dailyNorm/targetWeight тощо). Усі 115 profile-ключів × 3 мови — повні.
    - **Свідомо лишено UA в HTML:** `<title>`, unit-btn «кг/см» (disabled тех.одиниця).
    - ✅ **Дочищено (2026-06-27):** медичні дисклеймери ВЖЕ перекладені — кожен фрагмент обгорнуто в окремий `<span data-i18n>` (обхід проблеми вкладених `<strong>`/`<a>`); 8 ключів (`disclaimerBefore/NotMedical/NutritionWeight/Activity/Nutrition`, `weightWarningBefore/Link/After`) × 3 мови присутні в [i18n.js](js/i18n.js). streak «днів поспіль» теж локалізовано (`_streakLabel` у profile.js + `streakDaysLabel`/`t('streakSuffix')` у streak.js, 3 мови). Перевірено імпортом словника: усі 140 data-i18n профілю резолвляться в ua/pl/en.
  - 🟨 **Частина В — ДИНАМІЧНИЙ profile.js ЗРОБЛЕНО на 90% (2026-06-16).** Імпортовано `t` з i18n-apply. Локалізовано: streak-слово (`_streakLabel` — UA через pluralUA, PL/EN прості форми), 12 toast/повідомлень, BMI-категорії, дні тижня (масив за мовою), прийоми їжі, **18 видів активності `ACTIVITIES[].label`** (через t() — обчислюється раз при load, мова фіксована reload-стратегією), легенда БЖВ, 7 рекомендацій статистики (title+sub, з інтерпольованими), топи (empty/error), pfcShort/burnedKcal/series-names графіків, nickname-валідація, userDefault, noData/loadError. **+86 t()-ключів × 3 мови — усі повні.**
    - ✅ **Хвіст В — weight-advice ЗРОБЛЕНО (2026-06-16):** блок Персональні поради/Прогрес (BMI-поради ×4, ціль-поради ×4, прогрес-бар) локалізовано через ключі з плейсхолдерами `{bmi}/{min}/{max}/{dir}/{weeks}` (заміна `.replace()`); додано `kgUnit`. Заодно дочищено одиниці: легенда БЖВ (`г`), preview/balance калорій (`ккал`), fallback «Активність».
    - ✅ **Дочищено (2026-06-27):** одиниці в **форматерах графіків ApexCharts** (tooltip ` кг`/` г`/` ккал`/` разів`) ВЖЕ локалізовані через `t('kgUnit')`/`t('gramsLabel')`/`t('kcalShort')`/`t('timesUnit')` ([profile.js:257/263/641/867/932/1650/1656](js/profile.js#L257)) — хвіст закрито (раніше позначений як прийнятний UA-залишок).
  - **✅ ПРОФІЛЬ ПОВНІСТЮ ПЕРЕКЛАДЕНО:** статика (4а+4б) + динаміка (4в) + weight-advice. 109 t()-ключів у profile.js × 3 мови, усі повні. Знято тост «переклад профілю в роботі».
  - **🔍 Перевірка (2026-06-16, статична валідація + симуляція застосувача, без headless-браузера — playwright не встановлено):** усі data-i18n головної (57) і профілю (115) резолвляться у 3 мовах; симуляція виводу ключових елементів коректна. **Знайдено й виправлено 2 реальні баги:** (1) опції select активності показувались UA в PL/EN — `injectActivityIcons` мав умову `!querySelector('.nav-icon')`, що блокувала перезапис (HTML-опції вже з іконкою) → прибрано умову, тепер завжди оновлює з локалізованого `ACTIVITIES.label`; (2) `bodyweight` не мав ключа в ACTIVITIES → опція не перекладалась → додано `bodyweight` + `actBodyweight` ×3. Усі 12 опцій активності тепер локалізовані. ⏭️ дрібний хвіст: одиниці `г` у макро-барах index (×3, злиті з числом) + ApexCharts tooltip-форматери — інтернаціональні, не блокер.
  - **✅ Хвіст G по решті сторінок закрито (2026-06-17):** week-menu, recipes (список), cookbook і shopping-list тепер мають локалізовані `<title>`, `aria-label`, статичний HTML і динамічний JS-рендер (toast/confirm/empty state/cards/drawer/mobile week). Додано відсутні ключі в `i18n.js` для `ua/pl/en`. Статична перевірка (`node --check`) пройдена; live-браузер цього разу не пройдений, бо in-app runtime падав.
- **Інші сторінки:** ✅ week-menu, cookbook, recipes (список) — ЗРОБЛЕНО (2026-06-17).

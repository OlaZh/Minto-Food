...
- [x] ✅ **Undo action** — `lib/undoToast.ts`: тост 5 сек з кнопкою "Скасувати", потім виконує дію. Використовується: `useUndo` prop в `ActionButton` (Бан, Страйк, Відхилити, Приховати), в `ModerationReasonDialog` (Видалити рецепт, Бан з категорією)

### 🔮 Наступна черга — середня складність

- [x] ✅ **Auto-flagging** — автопідсвічування підозрілих сигналів в PUBLIC рецептах (не блокувати — тільки "увага, перевір"):
  - [x] ✅ 🚫 Inappropriate content — матюки, бридкий контент, треш
  - [x] ✅ 🔗 Suspicious links — казино, adult-links, phishing (generic URL не флагується — YouTube/Instagram легітимні)
  - [x] ~~🤖 Bot activity~~ — **видалено**: масовий import рецептів — нормальна поведінка (cookbook workflow)

...
async function init() {
  ...
  const { data: recipe, error } = await supabase
    .from('recipes')
    .select('id, name_ua, name_en, slug, image, kcal, protein, fat, carbs, steps, category, rating, user_id, created_at')
    .eq('slug', _slug)
    .eq('status', 'published')
    .is('deleted_at', null)
    .maybeSingle();
...
function _updateMeta(recipe, authorName) {
  ...
  const desc  = _buildDescription(recipe, authorName);
...
function _buildDescription(recipe, authorName) {
  const parts = [];
  ...
  const author   = authorName ? `Автор: ${authorName}.` : '';
...
function _injectSchemaOrg(recipe, authorName, ingredients) {
  ...
  const schema = {
    ...
    description: _buildDescription(recipe, authorName),

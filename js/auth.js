...
function updateAuthUI() {
  const profileNameEl = document.querySelector('.header__profile-name');
  ...
  if (currentUser) {
    ...
    if (authBtnEl) {
      authBtnEl.textContent = 'Вийти';
      authBtnEl.onclick = () => signOut();
    }
    ...
    // Не залогінений
    if (profileNameEl) {
      profileNameEl.textContent = 'Увійти';
      profileNameEl.href = '#';
      profileNameEl.removeEventListener('click', profileNameEl._authHandler);
      profileNameEl._authHandler = (e) => {
        e.preventDefault();
        e.stopPropagation();
        openAuthModal();
      };
      profileNameEl.addEventListener('click', profileNameEl._authHandler);
    }

    if (authBtnEl) {
      authBtnEl.textContent = 'Увійти';
      authBtnEl.onclick = () => openAuthModal();
    }
  }
}
        ...
        <div class="auth-modal__content" data-auth-content="login">
          <button class="auth-modal__google-btn" id="authGoogleBtn">
            <svg width="18" height="18" viewBox="0 0 24 24">
              ...
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 1２s.43 3.
...
export function openAuthModal(tab = 'login') {
  ...
  modal.removeAttribute('hidden'); // ← обов’язково!
  modal.style.display = 'flex';
  modal.style.position = 'fixed'; // ← критично важливо
  modal.style.inset = '0'; // або top/left/right/bottom = '0'
  modal.style.zIndex = '2000'; // або більше, якщо треба
...
export function closeAuthModal() {
  ...
  // modal.setAttribute('hidden', '');     ← можна додати назад, але не обов’язково

...
>
> **🎨 ОНОВЛЕНО (2026-06-27): візуальну уніфікацію ЗАКРИТО — але в ПРОТИЛЕЖНИЙ бік.** Замість зводити родину A до flat (як спершу припускав аудит), за рішенням власника **систему B підтягнуто ВГОРУ до вигляду родини A**: `%overlay`/`%modal-card` ([_design-system.scss](scss/utils/_design-system.scss)) озброєно radial-gradient фоном (`circle at top`, +dark-варіант), `blur(6px)` і градієнтною карткою (gradient surface + бордер + тінь `0 32px 64px`). Уся система B extend-ить ці плейсхолдери (з C4) → оновилась автоматично, **0 JS-змін, 0 правок кожної модалки**. Доведено дифом скомпільованого css: змінились лише `.report-overlay`/`.book-selector-overlay`/`.modal-overlay` + їхні картки; **родина A (`.modal`/`.scanner-modal`/`.auth-modal`) у дифі відсутня — не зачеплена.** Тобто весь видимий «3 системи виглядають по-різному» борг закрито. Лишається суто структурний хвіст: **JS-контракт показу родини A** (`hidden`-атрибут проти `.is-open` у B) — функціонально працює, не візуальний; окремий етап, якщо колись треба єдиний клас.

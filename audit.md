...
- 🟨 **Головна (index / меню на день)** — ІТЕРАЦІЯ 2 (2026-06-16): **статична розмітка ЗРОБЛЕНА.**
  ...
  - ⏭️ **Динамічний JS-рендер meals.js — НАСТУПНА ітерація:** toast'и, «Точно видалити?», «Продукти/Страви», «Нічого не знайдено», «Створити свій продукт», «Без назви» тощо будуються в JS (не data-i18n) → перекладати через імпорт `t()` з i18n-apply. Зачіпає найгарячіший файл щоденника — окремий обережний крок. Так само streak «N днів поспіль» (слово UA-хардкод).

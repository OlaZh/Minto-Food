...
- **Стан систем після цього кроку:** система B (короткі report/book-selector — власні overlay-класи; довгі recipe/detail/shop/cookbook — глобальний extend) вся на плейсхолдерах. Лишилось: **система A** (`.modal`/`hidden` — product/day/product-guide), **scanner** (пропущено, окрема структура), **auth/onboarding/admin** (окремі), і фінал **C3** (JS `.is-active`/`active`/`hidden`→єдиний `.is-open`).

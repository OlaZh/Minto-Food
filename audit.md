...
- 🟨 **Профіль (profile.html)** — ІТЕРАЦІЯ 4, частина А (2026-06-16): **вкладка «Налаштування» + перемикач ЗРОБЛЕНІ.**
  ...
  - ⏭️ **Частина В — ДИНАМІЧНИЙ profile.js (НАСТУПНИЙ крок):** streak-слово (pluralUA-форма), опції активностей (`ACTIVITIES[].label` + `injectActivityIcons` перезаписують HTML-текст), рендер історії/статистики/порад, toast'и. 2121 рядок — перекладати через `t()` (профіль уже імпортує getLang/setLang).

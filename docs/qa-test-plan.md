...
| AUTH-12 | PARTIAL FIX (was CRITICAL) | 06.08.2026, Ola; `https://minto-food.vercel.app/`; `f2dfb3d`+fix | Goal wizard пройдено повністю, дані НЕ збереглись у `user_profiles` (порожній профіль після wizard). Перевірено напряму в БД: RLS увімкнено, INSERT/UPDATE policy існують і виглядають коректно (`user_id = auth.uid()`) — отже причина НЕ відсутність policy, а щось інше (ймовірно auth-контекст запиту в момент upsert, або той самий race condition з AUTH-10/11, де другий паралельний `checkOnboarding()`-цикл переривав/дублював стан wizard-у). Застосовано фікс race condition (`_inFlight` lock, див. AUTH-10/11) — може вирішити і цю проблему опосередковано. Окремо виправлено мовчазний фейл: `_finish()` тепер показує `showToast(t('onbwSaveError'))` і НЕ закриває модалку при помилці upsert (`js/onboarding-wizard.js:350-355`), дані форми більше не губляться на invisible fail | screenshots профілю з порожніми полями; `pg_policies` вивід з живої БД; фікс race condition + user-facing error не переперевірені live | ОБОВ'ЯЗКОВО live-регрес: пройти wizard на новому акаунті, підтвердити що дані з'являються в профілі |
| PROFILE-02 (нотатка) | FAIL | 06.08.2026, Ola; `https://minto-food.vercel.app/`; `f2dfb3d`+fix | Кнопка "Змінити" біля нікнейму в Профіль→Налаштування→Акаунт не реагує на клік взагалі, без помилок у консолі | screenshot + консоль без помилок | N/A |
...
- [x] **AUTH-12:** PARTIAL FIX (was CRITICAL) — RLS policy на `user_profiles` виявились коректними (перевірено live), тож причина точно не в них; застосовано race-condition фікс (AUTH-10/11) + user-facing error toast замість мовчазного фейлу. ОБОВ'ЯЗКОВО live-регрес перед закриттям.

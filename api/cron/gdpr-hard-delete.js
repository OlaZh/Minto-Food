...
import { cleanupRecipeSources } from '../_lib/recipe-source-cleanup.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await cleanupRecipeSources(SUPABASE_URL, HEADERS);
  } catch (err) {
  ...
  for (const { id: userId } of dueUsers) {
    try {
      await cleanupRecipeSources(SUPABASE_URL, HEADERS, userId);

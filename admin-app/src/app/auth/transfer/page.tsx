...
import { validateTransferMessage, type TransferMessage } from '@/lib/security/transfer-message'
...
export default function TransferPage() {
  const configError = MAIN_SITE_ORIGIN
    ? null
    : 'Не налаштовано адресу основного сайту для secure transfer'
  ...
  useEffect(() => {
    ...
    async function handleMessage(event: MessageEvent<TransferMessage>) {
      const validation = validateTransferMessage({
      ...
      if (validation.status === 'error') {
        ...
        window.opener?.postMessage(
          { type: 'MINTO_ADMIN_SESSION_TRANSFER_ERROR', message },
          mainSiteOrigin
        )
        return
      }

      const { accessToken, refreshToken } = validation
      ...
      >
        ...
        {error ? (
          ...
          <p style={{ fontSize: '14px', lineHeight: 1.5, color: '#3f7558', margin: 0 }}>
            Переносимо безпечну сесію в адмінку. Це займе кілька секунд.
          </p>
        )}
      </div>
    </div>

# 🌿 MintoFood — Roadmap редизайну

> **Статус:** Старт квітень 2026  
> **Палітра:** Соковита м'ята (`#a6d6b8` / `#b8e0c5` / `#4ab584` / `#82bf99` / `#0f2818`)  
> **Принцип:** No MVP thinking. Робимо одразу правильно і фундаментально.  
> **НЕ чіпаємо:** шрифти (Fraunces/Rubik/Mulish), лого, існуючі кольори світлої та темної тем.

---

## 🧭 Форм-система проєкту

Правило premium-додатків (Apple Health, Noom, Lifesum): максимум **2 форми + 1 драматичний акцент**.

- **Rounded rectangles** (r=12–20px) — картки, кнопки, поля, контейнери
- **Pills** (r=999px) — чіпи фільтрів, дні тижня, streak-бейдж, прогрес-бари, водна капсула
- **Одне кільце на сторінку** — тільки для головного показника (наприклад, калорії на "Меню на день")

---

## 🎨 ФАЗА 0: Дизайн-система (фундамент)

**Форм-система:**
- [ ] Визначити стандарти rounded rectangles (r=12/16/20)
- [ ] Визначити стандарти pills (r=999)
- [ ] Правило "одне кільце на сторінку"

**SCSS компоненти (mixins/placeholders):**
- [ ] `%card-premium` — стандарт картки з hover
- [ ] `%button-primary` — основна кнопка
- [ ] `%button-secondary` — вторинна кнопка
- [ ] `%button-ghost` — прозора / утилітарна кнопка
- [ ] `%chip-filter` (категорії "Рецепти", "Путівник")
- [ ] `%chip-day` — дні тижня
- [ ] `%pill-badge` — streak, статуси
- [ ] `%progress-bar` — горизонтальний бар для макро
- [ ] `%ring-hero` — велике кільце калорій
- [ ] `%water-capsule` — вертикальна капсула води

**Spacing system:**
- [ ] Зафіксувати шкалу: 4, 8, 12, 16, 20, 24, 32, 40, 56 px
- [ ] Усі компоненти використовують тільки ці значення

**Shadow/elevation system:**
- [ ] Level 1 — картки (ледь піднято)
- [ ] Level 2 — hover state
- [ ] Level 3 — модалки, dropdowns

---

## 🏗 ФАЗА 1: Layout-система (каркас)

- [ ] Стандартний десктопний хедер (залишити як є)
- [ ] Стандартний підхедер: заголовок сторінки / breadcrumbs
- [ ] Мобільний хедер (компактний)
- [ ] Мобільний таб-бар: 5 іконок + "Ще" (6-та сторінка)
- [ ] "Ще" — bottom sheet або окрема сторінка
- [ ] Базовий 4-колонковий грід для dashboard
- [ ] Responsive breakpoints: 1200 / 1024 / 768 / 480 px

---

## 🏠 ФАЗА 2: Сторінка "Меню на день"

**Інфраструктура (готово):**
- [x] ✅ Таблиця `user_streaks` в Supabase
- [x] ✅ Функція `get_current_streak()`
- [x] ✅ Автотригер на `meals` для оновлення streak
- [x] ✅ Бекфіл історичних даних виконано

**Верстка:**
- [ ] Горизонтальні дні тижня (pills) під хедером
- [ ] Ліва колонка: велике кільце калорій + streak pill + швидкі дії
- [ ] Центральна колонка: прийоми їжі як аккордеони + разом за день
- [ ] Вузька колонка води: капсула + кнопки +0.25/+0.5/+0.75
- [ ] Права колонка: макро прогрес-бари (Б/Ж/В) + KБЖУ баланс
- [ ] Мобільна версія (1 колонка, таб-бар знизу)

**JS:**
- [ ] Streak-логіка (`js/streak.js` або в `stats.js`)
- [ ] Аккордеон-логіка для прийомів їжі
- [ ] Швидкі дії: "Копіювати день", "Очистити день"
- [ ] Тест світлої теми
- [ ] Тест темної теми

---

## 📅 ФАЗА 3: Сторінка "Меню на тиждень"

- [ ] Застосувати нові токени до матриці "дні × прийоми"
- [ ] Дні тижня як pills (консистентно з "Меню на день")
- [ ] Оновити "Разом" колонку під нову систему
- [ ] "Копіювати тиждень" / "Вставити тиждень" — оновити
- [ ] Мобільна версія (з галочками виконання, як на макеті)
- [ ] Тест світлої теми
- [ ] Тест темної теми

---

## 🍳 ФАЗА 4: Сторінка "Рецепти"

- [ ] Картки рецептів до нового стандарту
- [ ] Пошук + чіпи-фільтри під нові токени
- [ ] Рейтинги зірочками (перевірити/оновити)
- [ ] "+ Додати рецепт" — primary button
- [ ] Порожній стан (empty state)
- [ ] Мобільна версія (2 колонки карток)
- [ ] Тест світлої теми
- [ ] Тест темної теми

---

## 🥦 ФАЗА 5: Сторінка "Путівник по продуктах"

- [ ] Оновити картки продуктів (іконка + назва + макро + ккал)
- [ ] Пошук + чіпи фільтрів
- [ ] Розширені фільтри (Харч. цінність / Глікемічний індекс / Тип / Призначення / Обмеження)
- [ ] Модалка деталей продукту
- [ ] Мобільна версія (список, 1 колонка)
- [ ] Тест світлої теми
- [ ] Тест темної теми

---

## 🛒 ФАЗА 6: Сторінка "Список покупок"

- [ ] Групування по категоріях
- [ ] Чекбокси під нову систему
- [ ] Прогрес "4/9 куплено" — pill-стиль
- [ ] Права колонка дій — "Поділитися", "Очистити", "Друкувати"
- [ ] Додати продукт (швидкий ввід)
- [ ] Мобільна версія
- [ ] Тест світлої теми
- [ ] Тест темної теми

---

## 📚 ФАЗА 7: Сторінка "Книга рецептів"

- [ ] Оновити картки книг
- [ ] "Нещодавно переглянуті" збоку
- [ ] Модалка книги під форм-систему
- [ ] Нотатки, стікери — під нову систему
- [ ] Мобільна версія
- [ ] Тест світлої теми
- [ ] Тест темної теми

---

## 👤 ФАЗА 8: Профіль + підсторінки

- [ ] **Мої дані** — форма параметрів, Денна норма справа
- [ ] **Контроль ваги** — запис ваги, графік, прогрес до цілі
- [ ] **Активність** — додавання активностей, історія
- [ ] **Статистика** — діаграми, динаміка КБЖУ, топи, рекомендації
- [ ] **Налаштування** — нова сторінка (якщо потрібна)
- [ ] Streak в профілі (`longest_streak` показуємо тут)
- [ ] Мобільна версія всіх підсторінок
- [ ] Тест світлої теми
- [ ] Тест темної теми

---

## 🧭 ФАЗА 9: Навігація та авторизація

- [ ] Хедер — фінальний вигляд з аватаркою / login
- [ ] Модалка логіну/реєстрації — під нову систему
- [ ] Мобільний таб-бар — 5 іконок + "Ще"
- [ ] Сторінка/меню "Ще" — Путівник, Книга рецептів, Профіль

---

## ✅ ФАЗА 10: Фінальний поліш

- [ ] Повний огляд усіх сторінок у світлій темі
- [ ] Повний огляд усіх сторінок у темній темі
- [ ] Skeleton-loaders замість spinners
- [ ] Empty states на всіх сторінках
- [ ] Error states (коли API відвалився)
- [ ] Анімації переходів між станами
- [ ] Accessibility audit (focus states, ARIA, контраст)
- [ ] Performance audit (image lazy-load, CSS purge)
- [ ] Мобільний QA на реальних пристроях (iOS Safari, Android Chrome)

---

## 🧹 ФАЗА 11 (опційно): Чистка бази даних

- [ ] Розібратись з `profiles` vs `user_profiles` (мерджити?)
- [ ] Видалити `old_products` (якщо мертва таблиця)
- [ ] Видалити `recipetest` (якщо тестова)
- [ ] Розібратись з дублем `cookbook_notes` / `cookbook_n...`
- [ ] Аудит всіх RLS-політик

---

## 📝 Нотатки

- **Принцип "фундаментально":** ніяких стабів, плейсхолдерів, "потім допишемо". Кожен крок робимо як для production.
- **Тест двох тем:** кожна сторінка має одразу виглядати однаково premium і в світлій, і в темній.
- **Мобайл-first:** для нових компонентів спочатку продумуємо як виглядатиме на телефоні, потім масштабуємо на десктоп.
- **Не чіпаємо те, що працює:** існуюча логіка `meals.js`, `stats.js`, `auth.js` залишається. Міняємо тільки селектори CSS та HTML-розмітку.

---

*Останнє оновлення: квітень 2026*
